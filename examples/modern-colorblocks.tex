% !TeX program = xelatex
% 继承 claritycv.cls 默认风格，只覆盖现代色块风格与默认值不同的参数；全部信息均为虚构。
\documentclass{claritycv}
\definecolor{ExampleModernNavy}{RGB}{13,76,101}
\definecolor{ExampleModernBlue}{RGB}{14,137,190}
\resumesetup{
  color=ExampleModernNavy,
  body-size=8.8pt,
  section-line-width=0.8pt,
  entry-title-color=ExampleModernNavy,
  entry-subtitle-color=ExampleModernNavy,
  entry-banner-tint=7,
  entry-separator=bar
}
...
\resumeentry[style=banner,color=ExampleModernBlue,logo={images/logo-stellar-tech.png}]
...
\resumeentry[style=banner,color=ResumePink,logo={images/logo-cloud-lab.png}]
